...
fun codeLengthMinusDataLength(lines: List<String>): Int {
    return lines.map { it.trim() }.filter { it.isNotBlank() }.sumOf {
        val data = it.removeSurrounding("\"") // remove leading and trailing quotes
            .replace(
                Regex("\\\\x[a-f,0-9]{2}"), // replace hex escape sequences
                "a" // with the letter a
            )  // (I know this isn't correct, but I don't want to import Apache Commons Text just for this)
