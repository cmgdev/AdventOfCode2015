...
fun blinkenLights(instructions: List<String>): Int {
    val litLights = mutableSetOf<Pair<Int, Int>>()
        ...
        if (trim.isNotBlank()) {
            ...
            (start[0].toInt()..end[0].toInt()).forEach { x ->
                (start[1].toInt()..end[1].toInt()).forEach { y ->
                    val point = Pair(x, y)
                    when (parts[0]) {
                        "on" -> litLights.add(point)
                        "off" -> litLights.remove(point)
                        "toggle" -> {
                            if (litLights.contains(point)) litLights.remove(point)
                            else litLights.add(point)
                        }
                        else -> println("${parts[0]} is not a valid operation!")
                    }
                }
            }
        }
